...
func (book *OrderBook) checkAmountAndPrice(amount int32, price int32) error {
    if amount == int32(0) {
...
func (book *OrderBook) appendOrder(creator string, amount int32, price int32, ordering Ordering) (int32, error) {
   if err := book.checkAmountAndPrice(amount, price); err != nil {
    return 0, err
   }
...
func (book *OrderBook) InsertOrder(order Order, ordering Ordering) {
    if len(book.Orders) > 0 {
        var i int
        //get the index of new order depending on the provided ordering 
