...
func (im IBCModule) OnRecvPacket(
	...
	case *types.DexPacketData_SellPacket:
		packetAck, err := im.keeper.OnRecvSellPacket(ctx, modulePacket, *packet.SellPacket)
		if err != nil {
			ack = channeltypes.NewErrorAcknowledgement(err)
		} else {
			// Encode packet acknowledgment
		...
		ctx.EventManager().EmitEvent(
			sdk.NewEvent(
				types.EventTypeSellPacket,
				sdk.NewAttribute(sdk.AttributeKeyModule, types.ModuleName),
				sdk.NewAttribute(types.AttributeKeyAckSuccess, fmt.Sprintf("%t", err != nil)),
			),
		)
	case *types.DexPacketData_BuyPacket:
		packetAck, err := im.keeper.OnRecvBuyPacket(ctx, modulePacket, *packet.BuyPacket)
		if err != nil {
			ack = channeltypes.NewErrorAcknowledgement(err)
		} else {
			// Encode packet acknowledgment
...
func (im IBCModule) OnAcknowledgementPacket(
	...
	case *types.DexPacketData_SellPacket:
		err := im.keeper.OnAcknowledgementSellPacket(ctx, modulePacket, *packet.SellPacket, ack)
		if err != nil {
			return err
		}
		eventType = types.EventTypeSellPacket
	case *types.DexPacketData_BuyPacket:
		err := im.keeper.OnAcknowledgementBuyPacket(ctx, modulePacket, *packet.BuyPacket, ack)
		if err != nil {
			return err
		}
		eventType = types.EventTypeBuyPacket
...
func (im IBCModule) OnTimeoutPacket(
	...
	case *types.DexPacketData_SellPacket:
		err := im.keeper.OnTimeoutSellPacket(ctx, modulePacket, *packet.SellPacket)
		if err != nil {
			return err
		}
	case *types.DexPacketData_BuyPacket:
		err := im.keeper.OnTimeoutBuyPacket(ctx, modulePacket, *packet.BuyPacket)
		if err != nil {
			return err
		}
